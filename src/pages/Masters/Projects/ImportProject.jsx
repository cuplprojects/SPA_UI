...
const ImportProject = () => {
  ...
  const importImageConfigs = async (projectIdFrom, projectIdTo) => {
    try {
      const res = await axios.get(`${apiurl}/ImageConfigs/ByProjectId/${projectIdFrom}?WhichDatabase=${database}`);
      
      if (res.data.length > 0) {
        for (let i = 0; i < res.data.length; i++) {
          const imageConfig = res.data[i];
          imageConfig.projectId = projectIdTo; // Modify projectId to point to projectIdTo
          
          const postRes = await axios.post(`${apiurl}/ImageConfigs/?WhichDatabase=${database}`, imageConfig);
          console.log(`Successfully imported ImageConfig with ID ${imageConfig.id} to Project ${projectIdTo}`);
        }
        console.log(`All ImageConfigs imported successfully to Project ${projectIdTo}`);
      } else {
  ...
  const importFieldConfigs = async (projectIdFrom, projectIdTo) => {
    try {
      const res = await axios.get(`${apiurl}/FieldConfigurations/GetByProjectId/${projectIdFrom}?WhichDatabase=${database}`);
      
      if (res.data.length > 0) {
        for (let i = 0; i < res.data.length; i++) {
          const fieldConfig = res.data[i];
          fieldConfig.projectId = projectIdTo; // Modify projectId to point to projectIdTo
          fieldConfig.fieldConfigurationId = 0; // Assuming fieldConfig already has this field
          
          const postRes = await axios.post(`${apiurl}/FieldConfigurations/?WhichDatabase=${database}`, fieldConfig);
          console.log(`Successfully imported FieldConfig with ID ${fieldConfig.id} to Project ${projectIdTo}`);
        }
        console.log(`All FieldConfigs imported successfully to Project ${projectIdTo}`);
      } else {
        console.log(`No FieldConfigs found for Project ${projectIdFrom}`);
      }
    } catch (error) {
  ...
  const importResponseConfigs = async (projectIdFrom, projectIdTo) => {
    try {
      const res = await axios.get(`${apiurl}/ResponseConfigs/byproject/${projectIdFrom}?WhichDatabase=${database}`);
      ...
      if (res.data.length > 0) {
        for (let i = 0; i < res.data.length; i++) {
          ...
          const postRes = await axios.post(`${apiurl}/ResponseConfigs/byproject/${projectIdTo}?WhichDatabase=${database}`, responseConfig);
          console.log(`Successfully imported ResponseConfig with ID ${responseConfig.id} to Project ${projectIdTo}`);
        }
    ...
    <>
      ...
      >
        ...
        <div style={{ marginTop: '10px' }}>
          ...
          >
            {projects.filter(project => project.projectId !== importFrom).map(project => (
              <Option key={project.projectId} value={project.projectId}>
                {project.projectName}
              </Option>
            ))}
